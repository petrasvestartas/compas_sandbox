...
def ipopt_solver(**options):
    ...
    The IPOPT executable bundled with :mod:`compas_sandbox` is used when it is available,
    ...
    **options : dict, optional
    ...
    if exe is None:
        raise RuntimeError(
            "No IPOPT executable found. This installation of compas_sandbox does not "
            "bundle one (source installs and unsupported platforms do not), and none "
            "is available on PATH. Install a binary wheel with `pip install compas_sandbox` "
